...
function Session(options) {
  ...
  this._runningAutocomplete = false;
...
session.execCommandSet = function (wrapper, callback) {
  ...
  function runCommand(wrapper, inst, cb) {
    res = wrapper.fn.call(inst, wrapper.args, function () {
      var argus = util.fixArgsForApply(arguments);
      // wrapper.pipes.reduce((prev, cur) => {
      //   this.registerCommand();
      //   console.log('Curr command', cur.commandObject.name);
      //   const fn = cur.commandObject._fn || Promise.resolve();
      //   return prev.then((res) => {
      //     console.log('previous res', res);
      //     cur.args.previous = res;
      //     return fn.apply(cur, [cur.args])
      //   });
      // }, Promise.resolve()).then(() => {
      //
      // })
      cb(wrapper, argus[0], argus[1], argus);
    });
    ...
    if (res && _.isFunction(res.then)) {
      res.then(function (data) {
        if (wrapper.pipes.length === 0) {
          cb(wrapper, undefined, data);
          return;
        }
        ...
        wrapper.pipes.reduce((prev, cur) => {
          ...
          return prev.then(data => {
            cur.args.previous = data;
            return fn.apply(cur, [cur.args])
          });
        }, Promise.resolve(data))
        .then((data) => {
          cb(wrapper, undefined, data);
        }).catch(err => cb(wrapper, true, err));
  ...
  // Run the command before function
  if (_.isFunction(wrapper.before)) {
    wrapper.before.call(commandInstance, wrapper.args)
      .then(() => runCommand(wrapper, commandInstance, onCompletion))
      .catch(e => onCompletion(wrapper, e));
      return this;
  }

  // Call the root command.
  runCommand(wrapper, commandInstance, onCompletion)
