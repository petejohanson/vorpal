...
const util = {
  ...
  buildCommandArgs: function (passedArgs, cmd, execCommand, isCommandArgKeyPairNormalized) {
    ...
    for (let m = 0; m < cmd.options.length; ++m) {
      ...
      const longNormalized = _.camelCase(long);

      // Try to get property value from parsed arguments
      let exist = _.get(parsedArgs, short) || _.get(parsedArgs, long);

      // If option is missing and it's required return the help message
      const existsNotSet = (exist === true || exist === false);
      if (_.isUndefined(existsNotSet) && o.required !== 0) {
        return `\n  Missing required value for option ${(o.long || o.short)}. Showing Help:`;
      }

      // If options exists assign to arguments
      if (exist !== undefined) {

        // Optionally run a validation function
        if (_.isFunction(o.validation)) {
          exist = o.validation(exist);
        }

        // Assign property value
        args.options[longNormalized || short] = exist;

      } else {

        // If the default value exists
        if (_.isUndefined(o.default) === false) {
          args.options[longNormalized || short] = o.default;
        }
      }
    }
