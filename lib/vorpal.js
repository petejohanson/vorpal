...
function Vorpal() {
  ...
  this.session = new this.Session({local: true, user: 'local', parent: this, delimiter: this._delimiter});
  ...
  if (_.isFunction(commands)) {
    ...
    for (var i = 0; i < commands.length; ++i) {
      ...
      if (cmd.command) {
        ...
        if (cmd.options) {
          cmd.options = _.isArray(cmd.options)
            ? cmd.options
            : [cmd.options];
...
vorpal.command = function(name, desc, opts) {
  ...
  for (var i = 0; i < this.commands.length; ++i) {
    exists = (this.commands[i]._name === cmd._name)
      ? true
      : exists;
    if (exists) {
      this.commands[i] = cmd;
  ...
  if (!exists) {
    ...
    this.log(`Warning: command named "%s" was registered more than once.`, name);
    this.log(`If you intend to override a command, you should explicitly remove the first command with command.remove().`);
  }
...
vorpal._handleKeypress = function(data) {
  this.emit('keypress', data);
  this.parent._onKeypress(data.key, data.value);
};
 ...
 * @api public
 */
vorpal.show = function(showBanner) {
  if (this._banner !== '' && showBanner) {
    this.ui.log(VorpalUtil.padRow(this._banner));
  }
  ui.attach(this);
  ui.on('vorpal_ui_keypress', this._handleKeypress);
  return this;
};
...
vorpal.hide = function() {
  ui.detach(this);
  ui.removeListener('vorpal_ui_keypress', this._handleKeypress);
  return this;
};
...
vorpal.prompt = function(options = {}, userCallback) {
  return new Promise((resolve, reject) => {
    ...
    if (!ssn) {
      reject('Vorpal.prompt was called without a passed Session ID.');
      return;
    }
...
vorpal._exec = function(item) {
  ...
  item.command = (item.session._mode)
    ? item.session._mode
    : item.command;
  ...
  if (match) {
    ...
    if (item.sync === true) {
      ...
      var error;
      try {
        response = item.fn.call(new CommandInstance({downstream: undefined, commandWrapper: item, commandObject: item.commandObject, args: item.args}), item.args);
      } catch (e) {
        error = e;
      }
    ...
    item.pipes = item.pipes.map(function(pipe) {
      return new CommandInstance({commandWrapper: item, command: pipe.command._name, commandObject: pipe.command, args: pipe.args});
    });
...
vorpal._commandHelp = function(command) {
  ...
  var invalidString = (command && matches.length === 0 && singleMatches.length === 0)
  ...
  var groups = _.uniq(matches.filter(function(cmd) {
    ...
    let prefix = `    ${VorpalUtil.pad(cmd + ' *', width)}  ${counts[cmd]} sub-command${ (
      (counts[cmd] === 1)
      ? ''
      : 's')}.`;
    return prefix;
