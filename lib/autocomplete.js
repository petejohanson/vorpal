...
function parseInput(str, idx) {
  ...
  return ({raw: raw, prefix: prefix, suffix: suffix, context: context});
}
...
function getMatchData(input, cb) {
  ...
  /**
   * Using the provided confi data return the most likely match
   * the config can be an array, a function and a promise
   */
  function handleDataFormat(str, config, callback) {
    ...
    if (_.isArray(config)) {
      ...
      var cbk = (config.length < 2)
        ? (function() {})
        : (function(res) {
          callback(res || []);
        });
      var res = config.apply(self.parent, [str, cbk]);
      if (res && _.isFunction(res.then)) {
        // Return if still running previous autocomplete task
        // to prevent multiple function calls
        if (self._runningAutocomplete) {
          return;
        }
        // Set running flag to ON
        self._runningAutocomplete = true;
        res.then(function(resp) {
          callback(resp);
        }).catch(function(err) {
          callback(err);
        }). finally(() => {
          self._runningAutocomplete = false;
        });
